...
class entitas:
	def pilih(self,kalimat): #fungsi untuk menyatukan calon entitas (token dg kelas kata NN/NNP)
		...
		while i < len(kalimat):
			...
			if word in organisasi or word in lokasi or word in person :
				...
				temp = temp[:0]
			elif pos in ["NNP", "NN"] : #mengumpulkan kata dg kelas kata NN/NNP berturut2
				if i == (len(kalimat)-1): #menandai kata sebagai calon entitas yaitu kelas kata NN/NNP dan mempunya diawali huruf besar
					...
					if kalimat[i+1][1] in ["NNP", "NN"] :
						temp.append(word)
					elif kalimat[i+1][1] in ["NNP", "NN"]: #cek kata setelahnya klo NNP/NN masukkan list biar nyambung sama sebelumnya
						temp.append(word)
						ner.append([(temp, "entitas")])
						temp = temp[:0]
					else:
	...
	def aturan(self):
		...
		while i<len(tanggall): 
			...
			elif tanggall[i-1][0] in organisasi and tanggall[i][1] =="entitas": #aturan 1 semua
				hasil.append((word, "ORGANIZATION"))
		...
		while j<len(hasil):																	 #aturan 3 semua
			...
			if hasil[j][1] == "entitas":
				if hasil[j-1][1] == "CC" and hasil[j-2][1] == "ORGANIZATION":
					...
					hasil[j] = (word, "PERSON")
				 #aturan 2 semua
				elif hasil[j-1][1] == "," and hasil[j-2][1] == "ORGANIZATION":
					hasil[j] = (word, "ORGANIZATION")
				elif hasil[j-1][1] == "," and hasil[j-2][1] == "LOCATION":
					hasil[j] = (word, "LOCATION")
				elif hasil[j-1][1] == "," and hasil[j-2][1] == "PERSON":
					hasil[j] = (word, "PERSON")
					#aturan 4 semua
